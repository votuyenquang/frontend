...
export default function ProductDetails(){
    ...
    useEffect ( ()=> {
        const getReviewStar = async()=>{
            ...
            const res = await FetchAPI.postDataAPI("/review/getReviewByIdproduct",data);
            setarrReview(res)
    ...
    const ItemProductRelate = dataRelate.map((item,ind)=>{
        return(
            <Col key={ind} style={{display:'flex', justifyContent:'center',padding:"10px 10px" }}>
                <Product
                    width={200}
                    item={item}
                />
                             ...
                             <div className='review'>
                                <div className='review-block' key={ind}  >
                                    ...
                                    <span className='review-star'>
                                        <Rate  allowHalf style={{ color:"orange", fontSize:16}} tooltips="12345" defaultValue={review.reviewStar} disabled/>
                                    </span>
                                </div>    
